function Band({
  ...
  return (
    <div
      className={`flex h-20 w-max items-end whitespace-nowrap pb-3 pl-[10vw] will-change-transform ${
    ...
    >
      {items.map((_, i) => (
        <span
          key={i}
          className="mx-9 text-[clamp(1.95rem,2.55vw,2.65rem)] font-extrabold tracking-[0.08em] text-white drop-shadow"
        >
          {text}
        </span>
      ))}
...
export default function Marquee({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-[clamp(8rem,14vw,9.8rem)] overflow-hidden">
